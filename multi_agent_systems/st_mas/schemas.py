    ...
    for topic_name, topic_analysis in topics_data.items():
        ...
        for citation in exec_summary.get("citations", []):
            citation["article_sentence_citations"] = [
                enrich_citation_dict(c)
                for c in citation.get("article_sentence_citations", [])
            ]
...
def print_topic_summary(
    data: TopicSummaryAndAnalysis | dict, topic_name: str = "Topic Analysis"
) -> None:
    """
    ...
    for i, metric in enumerate(data.key_metrics, 1):
        ...
        if metric.citations:
            print(
                f"     📎 Sources: {', '.join(c.article_uuid for c in metric.citations)}"
            )
    ...
    for i, expert in enumerate(data.expert_analyses, 1):
        ...
        if expert.citations:
            print(
                f"     📎 Sources: {', '.join(c.article_uuid for c in expert.citations)}"
            )
    ...
    if data.executive_summary.citations:
        ...
        for i, citation in enumerate(data.executive_summary.citations, 1):
            ...
            for src in citation.article_sentence_citations:
                ...
                print(
                    f'      └─ Art. {src.article_uuid}{expert_str}: "{src.sentence[:80]}..."'
                    if len(src.sentence) > 80
                    else f'      └─ Art. {src.article_uuid}{expert_str}: "{src.sentence}"'
                )
