...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(self.style.WARNING("   (This analysis covers multiple topics in parallel, it might take a moment)"))
        ...
        try:
            ...
            # 3. Print Results
            self.stdout.write("\n" + "=" * 60)
            self.stdout.write(self.style.HTTP_INFO("📝 DETAILED TOPIC ANALYSIS RESULTS"))
            ...
            if save_to_db:
                ...
                self.stdout.write(self.style.SUCCESS(f"✅ Saved Analysis Group: {group_obj.uuid}"))
                self.stdout.write(self.style.HTTP_INFO(f"   ∟ contains {len(topic_analysis_collection.topics)} topics."))
