...
def create_payload(page_number, articles_count, api_key=os.getenv("EVENTREGISTRY_API_KEY"), date_start=None, date_end=None):
    ...
    if date_start and date_end:
        ...
        formatted_start = date_start.strftime("%Y-%m-%d") if hasattr(date_start, 'strftime') else date_start
        formatted_end = date_end.strftime("%Y-%m-%d") if hasattr(date_end, 'strftime') else date_end
...
def retrieve_articles(date_start, date_end, page_numbers=1, articles_count=5, api_key=None):
    url = "https://eventregistry.org/api/v1/article/"
    total_data = []
